...
const Settings = () => {
  ...
  const uploadFile = async () => {
    ...
    await deleteObject(mediaStorRef).then(() => console.log('file deleted successdully'))
  ...
  return (
    ...
    <form action="submit" onSubmit={(e) => sendData(e)} className="flex flex-col justify-center items-center gap-4">
      ...
      <label htmlFor="bio">
        Change Bio:
        <input type="text" name="bio" id="bio" value={bio || ''} onChange={(e) => handleBioChange(e)}/>
      </label>
      <button type="submit" className="bg-red-300">Save Changes</button>
    </form>
  </div>
  )
}
